...
The nightly ingest runs from cron, not `docker compose`. `/etc/cron.d/i-want-my-mtg` (installed from `cron/i-want-my-mtg`) runs `/opt/scripts/scry.sh` at **08:00 UTC daily**, appending to `/var/log/i-want-my-mtg/ingestion.log`. Sibling jobs: ingest retry (09:00 UTC -> `ingestion.log`), retention (Sun 03:00 UTC -> `retention.log`), published-deck ingest (daily 03:30 UTC, `ingest-decks --days 2` -> `decks.log`), price-alert processing (08:15 UTC -> `price-alerts.log`), portfolio-summary refresh (08:30 UTC -> `portfolio.log`), data-freshness check (10:00 UTC -> `health.log`), log cleanup (Sun 04:00 UTC -> `cleanup.log`). All times UTC; all jobs run as user `ubuntu`.

The 09:00 retry (`cron/ingest-retry.sh`) runs `scry health` first and only re-ingests when the catalog is actually stale, so a healthy night costs one query and mails nothing. It sits between the ingest and the 10:00 health check on purpose: a wedged run gets a second chance before the check that pages on it.
