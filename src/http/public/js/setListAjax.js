document.addEventListener('DOMContentLoaded', function () {
    ...
    function fetchAndRender(historyMethod) {
        ...
        if (resultsEl) {
            resultsEl.innerHTML = '<div class="text-center py-16"><i class="fas fa-spinner fa-spin text-4xl text-teal-500"></i></div>';
        }
    ...
    function showError(el, message) {
        ...
        el.innerHTML =
            ...
            '<p class="text-lg text-gray-600 dark:text-gray-400 font-medium mt-4">' + escapeHtml(message) + '</p>' +
            '</div>';
    }
    ...
    function renderTableHeaders() {
        ...
        html += renderSortableHeader({ key: 'set.releaseDate', label: 'Release Date', classes: 'xs-hide pr-2' });
        html += '</tr>';
        return html;
    }
    ...
    function renderSortableHeader(header) {
        ...
        var subtitleHtml = header.subtitle
            ...
            : '';
        var classAttr = 'table-header' + (header.classes ? ' ' + header.classes : '') + (header.key === 'set.name' ? ' pl-2' : '');

        return '<th class="' + classAttr + '">' +
            '<a href="?' + params.toString() + '" class="sort-btn">' +
            escapeHtml(header.label) + subtitleHtml +
    ...
    function renderSetRow(set) {
        ...
        if (authenticated) {
            ...
            if (set.ownedValue !== undefined && set.ownedValue !== null) {
                html += '<span class="text-sm font-medium text-gray-800 dark:text-gray-200">' + toDollar(set.ownedValue) + '</span>';
                html += '<div class="mt-0.5">' + renderCompletionBar(set.completionRate || 0) + '</div>';
            }
            html += '</td>';
        }
    ...
    function renderCompletionBar(rate) {
        return '<div class="completion-bar-container flex items-center relative">' +
            '<div class="completion-bar absolute top-0 left-0 h-full" style="width: ' + rate + '%;"></div>' +
            '<span class="w-full text-center z-10 font-bold relative">' + rate + '%</span>' +
            '</div>';
    }
    ...
    function renderPagination(meta) {
        ...
        if (page > 1) {
            html += '<a href="' + paginationHref(page - 1) + '" class="pagination-btn pagination-btn-primary">&lt;</a>';
            html += '<a href="' + paginationHref(1) + '" class="pagination-btn pagination-btn-tertiary">1</a>';
        }

        var skipBack = page - Math.floor(totalPages / 3);
        if (skipBack > 1 && skipBack < page) {
            html += '<a href="' + paginationHref(skipBack) + '" class="pagination-btn pagination-btn-tertiary">' + skipBack + '</a>';
            html += '<span class="text-gray-400 dark:text-gray-500">...</span>';
        }

        html += '<span class="pagination-btn pagination-btn-current" aria-current="page">' + page + '</span>';

        var skipForward = page + Math.floor(totalPages / 3);
        if (skipForward < totalPages && skipForward > page) {
            html += '<span class="text-gray-400 dark:text-gray-500">...</span>';
            html += '<a href="' + paginationHref(skipForward) + '" class="pagination-btn pagination-btn-tertiary">' + skipForward + '</a>';
        }

        if (page < totalPages) {
            html += '<a href="' + paginationHref(totalPages) + '" class="pagination-btn pagination-btn-tertiary">' + totalPages + '</a>';
            html += '<a href="' + paginationHref(page + 1) + '" class="pagination-btn pagination-btn-primary">&gt;</a>';
        }

        // Limit selector
        html += '<form method="get" action="/sets" class="flex items-center gap-2 mb-4 mt-2">';
        html += '<select id="limit" name="limit" class="input-field w-20 text-center py-1 pl-0 pr-2 text-xs sm:text-sm bg-white dark:bg-midnight-800 border border-teal-300 dark:border-teal-600 rounded-lg focus:ring-2 focus:ring-teal-400 focus:outline-none text-gray-900 dark:text-gray-100">';
        [25, 50, 100].forEach(function (val) {
            html += '<option value="' + val + '"' + (state.limit === val ? ' selected' : '') + '>' + val + '</option>';
        });
        html += '</select>';
        html += '<label for="limit" class="text-sm font-medium text-teal-700 dark:text-teal-300">per page</label>';
        html += '</form>';
