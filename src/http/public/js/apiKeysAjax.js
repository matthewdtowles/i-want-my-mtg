(function () {
    ...
    function loadUsage() {
        fetch('/api/v1/api-keys/usage', { credentials: 'same-origin' })
            ...
            .then(function (body) {
                ...
                var d = body.data;
                var pct = d.perDayLimit
                    ? Math.min(100, Math.round((d.todayCount / d.perDayLimit) * 100))
                    : 0;
                var bars = (d.history || [])
                    .map(function (h) {
                        var hPct = d.perDayLimit
                            ? Math.min(100, Math.round((h.count / d.perDayLimit) * 100))
                            : 0;
                        return (
                            '<div title="' +
                            escapeHtml(h.day) +
                    ...
                    .join('');
                usageEl.innerHTML =
                    '<h3 class="page-subtitle">Today\'s Usage</h3>' +
                    ...
                    (bars || '<span class="text-xs text-gray-500">No requests yet</span>') +
                    '</div></div>';
                if (pct >= 80) {
                    usageEl.insertAdjacentHTML(
                        'beforeend',
                        '<p class="mt-3 text-sm text-amber-700 dark:text-amber-400">You\'ve used ' +
                            pct +
    ...
    function renderKeys(keys) {
        var hasActive = keys.some(function (k) {
            return !k.revokedAt;
        });
        if (!keys.length) {
            listEl.innerHTML =
            ...
            var rows = keys
                .map(function (k) {
                    var status = k.revokedAt
                        ? '<span class="text-xs text-gray-500">Revoked ' +
                          fmtDate(k.revokedAt) +
                          '</span>'
                        : '<button class="btn btn-danger btn-sm" data-revoke-id="' +
                          k.id +
                          '">Revoke</button>';
                    return (
                        '<tr class="table-row">' +
                        ...
                        escapeHtml(k.name) +
                        '</td>' +
                        '<td class="table-cell"><code class="text-xs">' +
                ...
                .join('');
            listEl.innerHTML =
                '<table class="table-container w-full text-sm"><thead>' +
            ...
            .then(function (body) {
                if (!body.success) {
                    listEl.innerHTML =
                        ...
                        escapeHtml(body.error || 'Failed to load keys') +
                        '</p>';
                    return;
                }
    ...
    createForm.addEventListener('submit', function (e) {
        ...
        fetch('/api/v1/api-keys', {
            ...
            .then(function (res) {
                if (!res.body.success) {
                    if (window.showToast)
                        window.showToast(res.body.error || 'Failed to create key', 'error');
                    return;
                }
    ...
    listEl.addEventListener('click', function (e) {
        ...
        if (!btn) return;
        if (!confirm('Revoke this API key? Any clients using it will stop working immediately.'))
            return;
