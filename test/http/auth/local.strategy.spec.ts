...
describe('LocalStrategy', () => {
    ...
    it('should return the user when credentials are valid', async () => {
        ...
        const result = await localStrategy.validate('test@example.com', 'testPassword');

        expect(result).toEqual(mockUserDto);
        expect(authService.validateUser).toHaveBeenCalledWith('test@example.com', 'testPassword');
    });
