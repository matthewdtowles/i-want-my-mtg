...
import { getAuthCookieOptions } from 'src/http/hbs/auth/auth.cookie.util';
...
export class UserController {
    ...
    async update(
        ...
        @Req() req: AuthenticatedRequest
    ): Promise<ApiResponseDto<UserViewDto>> {
        try {
            ...
            return ApiResponseDto.okWithMessage(updatedUser, 'User updated');
        } catch (error) {
            const msg = `Error updating user ${httpUserDto?.email}: ${error?.message}`;
            this.LOGGER.error(msg);
            return ApiResponseDto.error(msg);
        }
    }
    ...
    async updatePassword(
        ...
        @Req() req: AuthenticatedRequest
    ): Promise<ApiResponseDto<BaseViewDto>> {
        try {
            ...
            return ApiResponseDto.okWithMessage(responseData, 'Password updated');
        } catch (error) {
    ...
    @UseGuards(JwtAuthGuard)
    @Delete()
    async remove(@Req() req: AuthenticatedRequest): Promise<ApiResponseDto<BaseViewDto>> {
        ...
        try {
            ...
            return ApiResponseDto.okWithMessage(response, 'User deleted');
        } catch (_error) {
