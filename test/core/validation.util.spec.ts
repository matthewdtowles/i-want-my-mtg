...
describe('validateInit', () => {
    ...
    it('should not throw when all required fields are present', () => {
        expect(() =>
            validateInit({ a: 1, b: 'test', c: true, d: 'unrequired value' } as TestType, [
                'a',
                'b',
            ])
        ).not.toThrow();
    ...
    it('should throw when a required field is missing', () => {
        expect(() => validateInit({ a: 1 } as TestType, ['a', 'b'])).toThrow(/b is required/);
    });

    it('should throw when a required field is null', () => {
        expect(() => validateInit({ a: 1, b: null } as TestType, ['a', 'b'])).toThrow(
            /b is required/
        );
    });

    it('should throw when a required field is undefined', () => {
        expect(() => validateInit({ a: 1, b: undefined } as TestType, ['a', 'b'])).toThrow(
            /b is required/
        );
    });
});
