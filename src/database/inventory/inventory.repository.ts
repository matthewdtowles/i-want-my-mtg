    ...
    implements InventoryRepositoryPort
{
    ...
    async findOne(userId: number, cardId: string, isFoil: boolean): Promise<Inventory | null> {
        ...
        this.LOGGER.debug(
            `Inventory item ${item ? 'found' : 'not found'} for userId: ${userId}, cardId: ${cardId}, isFoil: ${isFoil}.`
        );
    ...
    async findByCard(userId: number, cardId: string): Promise<Inventory[]> {
        ...
        this.LOGGER.debug(
            `Found ${items.length} inventory items for userId: ${userId}, cardId: ${cardId}.`
        );
    ...
    async findByCards(userId: number, cardIds: string[]): Promise<Inventory[]> {
        ...
        this.LOGGER.debug(
            `Found ${items.length} inventory items for userId: ${userId}, ${cardIds.length} cardIds.`
        );
        return items.map((item: InventoryOrmEntity) => InventoryMapper.toCore(item));
    }
    ...
    async totalInventoryValue(userId: number): Promise<number> {
        ...
        const totalInventoryValue = Number(queryResult[0]?.total_value ?? 0);
        this.LOGGER.debug(`User inventory value: ${totalInventoryValue}.`);
    ...
    async totalInventoryValueForSet(userId: number, setCode: string): Promise<number> {
        ...
        const totalInventoryCardsForSet = Number(queryResult[0]?.total_value ?? 0);
        this.LOGGER.debug(`User inventory value for set ${setCode}: ${totalInventoryCardsForSet}.`);
        return totalInventoryCardsForSet;
    }
    ...
    async delete(userId: number, cardId: string, foil: boolean): Promise<void> {
        ...
        try {
            await this.repository
                ...
                .execute();
            this.LOGGER.debug(
                `Deleted inventory item for userId: ${userId}, cardId: ${cardId}, foil: ${foil}.`
            );
        } catch (error) {
            this.LOGGER.error(
                `Failed to delete inventory item for userId: ${userId}, cardId: ${cardId}, foil: ${foil}. Error: ${error}`
            );
