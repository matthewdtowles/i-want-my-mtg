...
**`docs/ingest-runbook.md` is the copy-paste diagnostic guide** for a failed, hung, or stale ingest. Start there; this section is the layout.

The ingest is **hourly and gated**, not scheduled. `/etc/cron.d/i-want-my-mtg` (installed from `cron/i-want-my-mtg`) runs `/opt/scripts/ingest-chain.sh` at the top of every hour, logging to `/var/log/i-want-my-mtg/ingestion.log`. Sibling jobs: retention (Sun 03:00 UTC -> `retention.log`), published-deck ingest (daily 03:30 UTC -> `decks.log`), data-freshness check (10:00 UTC -> `health.log`), log cleanup (Sun 04:00 UTC -> `cleanup.log`). All times UTC; all jobs run as user `ubuntu`.

`ingest-chain.sh` runs `scry has-new-prices` first — a 200-byte Range request reading the `meta` key at the head of `AllPricesToday.json` — and exits silently unless upstream is serving data newer than ours (exit 0 = new, 3 = current, 1 = could not tell, which fails closed). When there is new data it runs ingest, then price alerts, then portfolio summary, **sequentially**, because each depends on the one before it.

**Do not put these back on fixed times.** That is what it replaced — ingest 08:00, price alerts 08:15, portfolio 08:30, retry 09:00 — and every one of those times encoded a guess about when MTGJSON publishes. It does not publish predictably: a probe polling every 10 minutes from 2026-08-28 saw no new build for 22 hours, nine of them past the assumed window. The 08:00 run downloaded 550MB daily to rediscover the previous day's data and mailed about it, and the 09:00 retry never fired because `scry health` tolerates a one-day-old catalog.

**Read the date from `AllPricesToday.json`, never from `Meta.json` or `Last-Modified`.** The bulk file's own `meta.date` is what decides the data we store; the others can disagree with it. On 2026-08-28 `Last-Modified` read 06:08 UTC while a download at 08:03 still returned the previous day's prices — which is how the 08:00 slot was chosen and why it was wrong.

**Four nested deadlines bound a wedge**, and which one fires tells you what happened. Innermost first: Postgres `lock_timeout` (30s) and `statement_timeout` (10 min), set by scry on every connection it opens; scry's own whole-command deadline (`SCRY_COMMAND_TIMEOUT_SECONDS`, 1800s), which logs the phase it overran in; and `scry.sh`'s `timeout` (`SCRY_TIMEOUT_SECONDS`, 2400s), which SIGTERMs and tells you nothing. Before 2026-08-22 only the outermost existed, and the managed instance leaves `statement_timeout` and `lock_timeout` at 0 - so a query waiting on a row lock (an FK check on `card` from a web inventory or deck write is enough) waited forever. That is exactly what the 2026-08-22 ingest did: it stopped mid-card-stream at 08:01, sat at 98% idle CPU for 59 minutes, and was killed at 09:00 having logged nothing since, leaving the catalog on 2026-08-20 prices for two days.

**A failed ingest no longer prunes.** The post-ingest prunes decide what to delete from what is in the database *now*, on the assumption the ingest just brought it up to date; over a half-written catalog they are destructive. The 2026-08-22 run had written the set list and then died partway through the card stream, so every set alphabetically after the failure point held zero cards - precisely what `prune_empty_sets` deletes. Only the stale-row sweep was guarded (via the ingest ledger); the four policy prunes were not. They are now skipped, along with post-ingest updates, whenever the ingest itself failed.
